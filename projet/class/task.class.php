...
class Task extends CommonObjectLine
{
	...
	public function addTimeSpent($user, $notrigger = 0)
	{
		...
		$sql = "SELECT dateo, datee FROM " . MAIN_DB_PREFIX . "projet";
		$sql .= " WHERE rowid = " . (int) $this->timespent_projectId;

		$res = $this->db->query($sql);
		if ($res) {
			$projetData = $this->db->fetch_object($res);
			if ($projetData) {
				$dateo = $projetData->dateo;
				$datee = $projetData->datee;
			} else {
			...
			dol_syslog(get_class($this) . "::addTimeSpent - Error querying project data", LOG_ERR);
			$dateo = null;
			$datee = null;
		}
		$this->timespent_date = date('Y-m-d', $this->timespent_date);

		//echo "dateo ". $dateo. " dateee ". $datee . " selected date ".$this->timespent_date;
		// Validate timesheet entry date against project date range
		if ($this->timespent_date < $dateo || $this->timespent_date > $datee) {
		    $formattedDateo = strtotime($dateo);
		    $formattedDateo = date("d-F-Y", $formattedDateo);


		    $formattedDatee = strtotime($datee);
		    $formattedDatee = date("d-F-Y", $formattedDatee);
		    $this->error = "Timesheet entry date must be within " . $formattedDateo .  " and " .$formattedDatee ;
		    $this->errors[] = $this->error;
		    return -1;
		}
		$this->db->begin();
	...
	public function updateTimeSpent($user, $notrigger = 0)
	{
		...
		$timespent->comments = $this->timespent_comments;
		$timespent->ot_pay = $this->timespent_ot_pay;
		$timespent->timespent_projectId = $this->timespent_projectId;
		$timespent->km_pay = $this->timespent_km_pay;
		$timespent->overhead_work = $this->timespent_overhead_work;
		$timespent->accommodation = $this->timespent_accommodation;
		$timespent->local_night_out = $this->timespent_local_night_out;
