...
if ($action == 'create') {
    ...
    // SELECT total_tva, localtax1, localtax2, total_ht, total_ttc, revenuestamp From llx_facture where fk_soc = 1 and fk_projet = 4;
    ...
    if ($res_llx_propal) {
        while ($row = $db->fetch_object($res_llx_propal)) {
            ...
            SET total_ht = '" . $db->escape($object->total_ht) . "', 
                total_tva = '" . $db->escape($object->total_tva) . "', 
                total_ttc = '" . $db->escape($object->total_ttc) . "' 
            WHERE fk_soc = $object->socid AND fk_projet = $object->fk_project";

            // Execute the update query
            $res_update_facture = $db->query($sql_update_facture);
            if (!$res_update_facture) {
                // Handle the error in updating facture
                echo "Error updating facture: " . $db->lasterror();
            }
        }
    }
