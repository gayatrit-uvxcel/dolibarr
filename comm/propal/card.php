...
if (empty($reshook)) {
    ...
    if ($action == 'confirm_clone' && $confirm == 'yes' && $usercancreate) {
        ...
        // added by gt insetad of datep updating dateOfProposal
        $dateOfProposal = dol_mktime(12, 0, 0, GETPOST('dateOfProposalmonth'), GETPOST('dateOfProposalday'), GETPOST('dateOfProposalyear'));
        // $datep = dol_mktime(12, 0, 0, GETPOST('remonth', 'int'), GETPOST('reday', 'int'), GETPOST('reyear', 'int'));

        if (empty($dateOfProposal)) {
            $error++;
        ...
        if (!$error) {
            ...
            // changed by gt
            $result = $object->set_date($user, $dateOfProposal);
            if ($result > 0 && !empty($object->duree_validite) && !empty($object->fin_validite)) {
                $datev = $dateOfProposal + ($object->duree_validite * 24 * 3600);
        ...
        if (!$error) {
            ...
            if (GETPOST('createmode') == 'copy' && GETPOST('copie_propal')) {
                if ($object->fetch(GETPOST('copie_propal', 'int')) > 0) {
                    ...
                    $object->date = $datep;
                    // added by gt
                    $object->dateOfProposal = $dateOfProposal;
                ...
                if (isModEnabled("settermsandconditions")) {
                    $object->terms_and_conditions = GETPOST('$selected_terms_json', 'alpha');
                }
    ...
    if ($action == 'modif' && $usercancreate) {
        ...
        if (!$error) {
            ...
            $result = $object->updateline(GETPOST('lineid', 'int'), $pu, $qty, $remise_percent, $vat_rate, $localtax1_rate, $localtax2_rate, $description, $price_base_type, $info_bits, $special_code, GETPOST('fk_parent_line'), 0, $fournprice, $buyingprice, $label, $type, $category, $date_start, $date_end, $array_options, GETPOST("units"), $pu_ht_devise, $selected_terms_json);
    ...
    } elseif ($action == 'settermsandconditions' && $usercancreate) {
        // terms and conditions Code
        $result = $object->setTermsAndConditions(GETPOST('$selected_terms_json', 'alpha'));
        $selected_terms = GETPOST('multi_tc_content');
        $selected_terms_json = json_encode($selected_terms);
        $object->terms_and_conditions = $selected_terms_json;
        $sql = "UPDATE llx_propal SET terms_and_conditions = '" . $db->escape($selected_terms_json) . "' WHERE rowid = " . $object->id;
        $resql = $db->query($sql);
        if ($resql) {
            // Update the object as well
            $object->terms_and_conditions = $selected_terms_json;
        } else {
            echo json_encode(array('status' => 'error', 'message' => 'Failed to update terms and conditions.'));
        }
    } elseif ($action == 'setnote' && $usercancreate) {
        // notes Code
        $result = $object->setNote(GETPOST('$selected_terms_json', 'alpha'));
        $selected_notes = GETPOST('notes_content');
        $selected_terms_json = json_encode($selected_notes);
        $object->notes = $selected_terms_json;
        $sql = "UPDATE llx_propal SET notes = '" . $db->escape($selected_terms_json) . "' WHERE rowid = " . $object->id;
        $resql = $db->query($sql);
        if ($resql) {
            // Update the object as well
            $object->notes = $selected_terms_json;
...
if ($action == 'create') {
    ...
    // Date of enquiry
    print '<tr class="field_addprop"><td class="titlefieldcreate fieldrequired">' . 'Date Of Enquiry' . '</td><td class="valuefieldcreate">';
    ...
    print img_picto('', 'action', 'class="pictofixedwidth"');
    print $form->selectDate('', 'dateOfProposal', '', '', '', "addprop", 1, 1);
    print '</td></tr>';
    ...
    if ($socid > 0) {
        ...
        if (isModEnabled('project')) {
            ...
            print img_picto('', 'project', 'class="pictofixedwidth"') . $formproject->select_projects(-1, $projectid, 'projectid', 0, 0, 1, 1, 0, 0, 0, '', 1, 0, 'maxwidth500 widthcentpercentminusxx');
    ...
    // Date of proposal changed to enquiry by gt
    print '<tr>';
    print '<td>';
    print '<table class="nobordernopadding" width="100%"><tr><td>';
    print $langs->trans('Date of Enquiry');
    print '</td>';
    ...
    print '<td>';
    print '<table class="nobordernopadding" width="100%"><tr><td>';
    print $langs->trans('Date of Proposal');
    print '</td>';
    if ($action != 'editdate' && $object->statut == $object::STATUS_DRAFT && $usercancreate && $caneditfield) {
        print '<td class="right"><a class="editfielda" href="' . $_SERVER["PHP_SELF"] . '?action=editdate&token=' . newToken() . '&id=' . $object->id . '">' . img_edit($langs->trans('SetDate'), 1) . '</a></td>';
    }
    ...
    if ($action == 'editdate' && $usercancreate && $caneditfield) {
        ...
        print '<input type="hidden" name="backtopage" value="' . $backtopage . '">';
        print '<input type="hidden" name="modifiedQuotationNo" value="' . $modifiedQuotationNo . '">';
        print $form->selectDate($object->dateOfProposal, 'dateOfProposal', '', '', 0, "editdate");
        print '<input type="submit" class="button button-edit" value="' . $langs->trans('Modify') . '">';
        print '</form>';
    } else {
        print dol_print_date($object->dateOfProposal, 'day');
    }
    print '<td>';
    // date of proposal added by gt
    // Date of proposal
    ...
    print '<td>';
    print '<table class="nobordernopadding" width="100%"><tr><td>';
    print $form->editfieldkey('Terms and Conditions', 'terms_and_conditions', '', $object, 0);
    print '</td>';
    if ($action != 'edittermsandconditions' && $object->statut == $object::STATUS_DRAFT && $usercancreate) {
        print '<td class="right"><a class="editfielda" href="' . $_SERVER["PHP_SELF"] . '?action=edittermsandconditions&token=' . newToken() . '&id=' . $object->id . '">' . img_edit($langs->transnoentitiesnoconv('Set Terms and Conditions'), 1) . '</a></td>';
    }
    print '</tr></table>';
    print '</td><td class="valuefield">';

    $terms_and_conditions_array = json_decode($object->terms_and_conditions, true);

    if ($object->statut == $object::STATUS_DRAFT && $action == 'edittermsandconditions' && $usercancreate) {
        $form->form_terms_conditions($_SERVER['PHP_SELF'] . '?id=' . $object->id, $object->terms_and_conditions, 'terms_and_conditions');
    } else {
        // $form->form_terms_conditions($_SERVER['PHP_SELF'] . '?id=' . $object->id, $terms_and_conditions_array, 'none');
        if (!empty($terms_and_conditions_array)) {
            foreach ($terms_and_conditions_array as $key => $value) {
                echo '<li>';
                echo '<input type="hidden" name="terms_and_conditions[]" value="' . htmlspecialchars($key) . '">';
    ...
    print '<td>';
    print '<table class="nobordernopadding" width="100%"><tr><td>';
    print $form->editfieldkey('Notes', 'notes', '', $object, 0);
    print '</td>';
    if ($action != 'editnote' && $object->statut == $object::STATUS_DRAFT && $usercancreate) {
        print '<td class="right"><a class="editfielda" href="' . $_SERVER["PHP_SELF"] . '?action=editnote&token=' . newToken() . '&id=' . $object->id . '">' . img_edit($langs->transnoentitiesnoconv('Set Terms and Conditions'), 1) . '</a></td>';
    }
    print '</tr></table>';
    print '</td><td class="valuefield">';

    $terms_and_conditions_array = json_decode($object->notes, true);

    if ($object->statut == $object::STATUS_DRAFT && $action == 'editnote' && $usercancreate) {
        $form->form_notes($_SERVER['PHP_SELF'] . '?id=' . $object->id, $object->notes, 'notes');
    } else {
        // $form->form_terms_conditions($_SERVER['PHP_SELF'] . '?id=' . $object->id, $terms_and_conditions_array, 'none');
        if (!empty($terms_and_conditions_array)) {
            foreach ($terms_and_conditions_array as $key => $value) {
                echo '<li>';
                echo '<input type="hidden" name="notes[]" value="' . htmlspecialchars($key) . '">';
                echo '<input type="text" value="' . htmlspecialchars($value) . '" disabled>';
                echo '</li>';
            }
        } else {
            echo 'No note available.';
        }
    }
    print '</td></tr>';
