...
if (empty($reshook)) {
    ...
    if ($action == 'settermsandconditions' && $usercancreate) {
        ...
        $sql ="UPDATE llx_commande_fournisseur SET terms_and_conditions = '".$db->escape($selected_terms_json)."' WHERE rowid = ".$object->id;
        $resql = $db->query($sql);
        if($resql) {        
...
if ($action == 'setnote' && $usercancreate) {
    // notes Code
    ...
    $sql ="UPDATE llx_commande_fournisseur SET notes = '".$db->escape($selected_terms_json)."' WHERE rowid = ".$object->id;
    $resql = $db->query($sql);
    if($resql) {        
        // Update the object as well
        $object->notes = $selected_terms_json;        
...
if ($action == 'create') {
    ...
    if (isModEnabled("multicurrency")) {
     ...
     if ($object->statut == $object::STATUS_DRAFT && $action == 'edittermsandconditions' && $usercancreate) {
         $form->form_terms_conditions_po($_SERVER['PHP_SELF'] . '?id=' . $object->id, $object->terms_and_conditions, 'terms_and_conditions');
     } else {
...
if ($object->statut == $object::STATUS_DRAFT && $action == 'editnote' && $usercancreate) {
    $form->form_notes_po($_SERVER['PHP_SELF'] . '?id=' . $object->id, $object->notes, 'notes');
} else {
    // $form->form_terms_conditions($_SERVER['PHP_SELF'] . '?id=' . $object->id, $terms_and_conditions_array, 'none');
    if (!empty($terms_and_conditions_array)) {
        foreach ($terms_and_conditions_array as $key => $value) {
