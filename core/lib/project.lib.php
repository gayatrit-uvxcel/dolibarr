		...
		if ($lines[$i]->fk_task_parent == $parent) {
			...
			if (empty($mine) || !empty($tasksrole[$lines[$i]->id])) {
				...
				if (empty($oldprojectforbreak) || ($oldprojectforbreak != -1 && $oldprojectforbreak != $projectstatic->id)) {
					...
					print '<tr class="oddeven trforbreak nobold">' . "\n";
					print '<td colspan="' . (23 + $addcolspan) . '">';
				...
				*/
				...
				// projectid
				print '<input type="hidden" name="' . $lines[$i]->id . 'projectId" value="' . $projectstatic->id . '">';
				...
				$selected_default = (GETPOST($lines[$i]->id . 'leave_information') == '') ? ' selected' : '';
				print '<option value="" disabled' . $selected_default . '>Leave Information</option>';


				$options = [
					// "FVA-ANN-LEAVE" => "FVA-ANN-LEAVE",
					// "FVA-UNPAID-LEAVE" => "FVA-UNPAID-LEAVE",
					// "FVA-PUBLIC-HOLIDAY" => "FVA-PUBLIC-HOLIDAY",
					// "FVA-SICK-LEAVE" => "FVA-SICK-LEAVE",
					"FVA-WORKING" => "FVA-WORKING",
				];

				foreach ($options as $value => $label) {
					$selected = (GETPOST($lines[$i]->id . 'leave_information') == $value) ? ' selected' : '';
				...
				$tableCell = '';
				$tableCell .= '<span class="timesheetalreadyrecorded" title="texttoreplace">
				<input type="text" class="center" size="2" disabled id="timespent[' . $inc . '][' . $idw . ']" name="task[' . $lines[$i]->id . '][' . $idw . ']" value="' . $alreadyspent . '"></span>';
				...
				$selected_default = (GETPOST($lines[$i]->id . 'main_activity') == '') ? ' selected' : '';
				print '<option value="" disabled' . $selected_default . '>Main Activity</option>';


				$options = [
					"Meeting Client" => "Meeting Client",
					"Meeting Technical Clarification" => "Meeting Technical Clarification",
					"Meeting Tender Discussion" => "Meeting Tender Discussion",
					"Meeting Commercial Discussion" => "Meeting Commercial Discussion",
					"Site Specific Inductions" => "Site Specific Inductions",
					"Site Specific Medicals" => "Site Specific Medicals",
					"In Office Work" => "In Office Work",
					"In Office Training" => "In Office Training",
					"Instrumentation Design" => "Instrumentation Design",
					"Instrumentation IO Count" => "Instrumentation IO Count",
					"Instrumentation P&ID Review" => "Instrumentation P&ID Review",
					"Instrumentation Drawing CAD Review" => "Instrumentation Drawing CAD Review",
					"Instrumentation Datasheet" => "Instrumentation Datasheet",
					"Instrumentation Specification Document" => "Instrumentation Specification Document",
					"Site Acceptance Test - Panel" => "Site Acceptance Test - Panel",
					"Instrumentation Vendor Adjudication" => "Instrumentation Vendor Adjudication",
					"Instrumentation Ordering" => "Instrumentation Ordering",
					"Instrumentation Installation" => "Instrumentation Installation",
					"Instrumentation IO Testing" => "Instrumentation IO Testing",
					"Instrumentation Commissioning" => "Instrumentation Commissioning",
					"Instrumentation Handover" => "Instrumentation Handover",
					"Instrumentation Handover Signoff" => "Instrumentation Handover Signoff",
					"Control System Design" => "Control System Design",
					"Control System IO Count" => "Control System IO Count",
					"Control System P&ID Review" => "Control System P&ID Review",
					"Control System Specification Document" => "Control System Specification Document",
					"Control System Enquiry Vendors" => "Control System Enquiry Vendors",
					"Control System Vendor Adjudication" => "Control System Vendor Adjudication",
					"Control System Ordering" => "Control System Ordering",
					"Control System Installation" => "Control System Installation",
					"Control System IO Testing" => "Control System IO Testing",
					"Control System Commissioning" => "Control System Commissioning",
					"Control System Handover" => "Control System Handover",
					"Control System Handover Signoff" => "Control System Handover Signoff",
					"Control System Programming Office" => "Control System Programming Office",
					"Control System Programming Site" => "Control System Programming Site",
					"Factory Acceptance Test - Panel" => "Factory Acceptance Test - Panel",
					"Factory Acceptance Test - Hardware" => "Factory Acceptance Test - Hardware",
					"Factory Acceptance Test - Software" => "Factory Acceptance Test - Software",
					"Site Acceptance Test - Panel" => "Site Acceptance Test - Panel",
					"Site Acceptance Test - Software" => "Site Acceptance Test - Software",
					"Commissioning Cold" => "Commissioning Cold",
					"Commissioning Hot" => "Commissioning Hot",
					"Schematic Diagram Drawing" => "Schematic Diagram Drawing",
					"Intervention (Breakdown, Adhoc Service)" => "Intervention (Breakdown, Adhoc Service)",
					"Intervention (Remote Access,etc)" => "Intervention (Remote Access,etc)",

				];

				foreach ($options as $value => $label) {
					// Check if this option should be selected
					$selected = (GETPOST($lines[$i]->id . 'main_activity') == $value) ? ' selected' : '';
					print '<option value="' . $value . '"' . $selected . '>' . $label . '</option>';
				}
				print '</select>';
				print '</td>';
				...
				print '<td class="center">';
				print '<textarea name="' . $lines[$i]->id . 'note" rows="' . ROWS_2 . '" id="' . $lines[$i]->id . 'note"' . ($disabledtask ? ' disabled="disabled"' : '') . '>';
				print htmlspecialchars(GETPOST($lines[$i]->id . 'note'));
				print '</textarea>';
				print '</td>';

				// Over Time
				print '<td class="center">';
				print '<input type="number" name="' . $lines[$i]->id . 'over_time" id="' . $lines[$i]->id . 'over_time" value="' . htmlspecialchars(GETPOST($lines[$i]->id . 'over_time')) . '"' . ($disabledtask ? ' disabled="disabled"' : '') . 'step="0.01">';
				print '</td>';

				// Night Out
				print '<td class="center">';
				print '<input type="number" name="' . $lines[$i]->id . 'night_out" rows="' . ROWS_2 . '" id="' . $lines[$i]->id . 'night_out"value="' . htmlspecialchars(GETPOST($lines[$i]->id . 'night_out')) . '"' . ($disabledtask ? ' disabled="disabled"' : '') . 'step="0.01" >';
				print '</input>';
				print '</td>';

				// Travel KM
				print '<td class="center">';
				print '<input type="number" name="' . $lines[$i]->id . 'travel_km" rows="' . ROWS_2 . '" id="' . $lines[$i]->id . 'travel_km" value="' . htmlspecialchars(GETPOST($lines[$i]->id . 'travel_km')) . '"' . ($disabledtask ? ' disabled="disabled"' : '') . 'step="0.01">';
				print '</input>';
				print '</td>';
				...
				print htmlspecialchars(GETPOST($lines[$i]->id . 'comments'));
				print '</textarea>';
				print '</td>';

				//OT Pay
				print '<td class="center">';
				print '<div class="custom-input-group"><span class="input-group-text">R</span><input  class="form-control" type="number" name="' . $lines[$i]->id . 'ot_pay" rows="' . ROWS_2 . '" id="' . $lines[$i]->id . 'ot_pay" value="' . htmlspecialchars(GETPOST($lines[$i]->id . 'ot_pay')) . '"' . ($disabledtask ? ' disabled="disabled"' : '') . 'step="0.01" >';
				print '</input></div>';
				print '</td>';
				...
				//KM pay
				print '<td class="center">';
				print '<div class="custom-input-group"><span class="input-group-text">R</span><input class="form-control" type="number"  name="' . $lines[$i]->id . 'km_pay" rows="' . ROWS_2 . '" id="' . $lines[$i]->id . 'km_pay" value="' . htmlspecialchars(GETPOST($lines[$i]->id . 'km_pay')) . '"' . ($disabledtask ? ' disabled="disabled"' : '') . 'step="0.01" >';
				print '</input></div>';
				print '</td>';

				//Overhead work
				$selected_overhead_work = GETPOST($lines[$i]->id . 'overhead_work');
				print '<td class="center">';
				print '<select name="' . $lines[$i]->id . 'overhead_work" rows="' . ROWS_2 . '" id="' . $lines[$i]->id . 'overhead_work"' . ($disabledtask ? ' disabled="disabled"' : '') . '>';
				print '<option value="FVA-ANN-LEAVE" disabled' . (empty($selected_overhead_work) ? ' selected' : '') . '>Select Overhead Work</option>';
				print '<option value="YES"' . ($selected_overhead_work == 'YES' ? ' selected' : '') . '>YES</option>';
				print '<option value="NO"' . ($selected_overhead_work == 'NO' ? ' selected' : '') . '>NO</option>';
				print '</select>';
				print '</td>';

				//Accommodation
				print '<td class="center">';
				print '<div class="custom-input-group"><span class="input-group-text">R</span><input class="form-control" type="number" name="' . $lines[$i]->id . 'accommodation" rows="' . ROWS_2 . '" id="' . $lines[$i]->id . 'accommodation" value="' . htmlspecialchars(GETPOST($lines[$i]->id . 'accommodation')) . '"' . ($disabledtask ? ' disabled="disabled"' : '') . 'step="0.01" >';
				print '</input></div>';
				print '</td>';

				//Local Night Out
				print '<td class="center">';
				print '<input type="number" name="' . $lines[$i]->id . 'local_night_out" rows="' . ROWS_2 . '" id="' . $lines[$i]->id . 'local_night_out" value="' . htmlspecialchars(GETPOST($lines[$i]->id . 'local_night_out')) . '"' . ($disabledtask ? ' disabled="disabled"' : '') . 'step="0.01" >';
				print '</input>';
				print '</td>';

				//Intal Night Out
				print '<td class="center">';
				print '<input type="number" name="' . $lines[$i]->id . 'nital_night_out" rows="' . ROWS_2 . '" id="' . $lines[$i]->id . 'nital_night_out" value="' . htmlspecialchars(GETPOST($lines[$i]->id . 'nital_night_out')) . '"' . ($disabledtask ? ' disabled="disabled"' : '') . 'step="0.01" >';
				print '</input>';
				print '</td>';

				//Night Out Allowance
				print '<td class="center">';
				print '<div class="custom-input-group"><span class="input-group-text">R</span><input class="form-control" type="number" name="' . $lines[$i]->id . 'night_out_allowance" rows="' . ROWS_2 . '" id="' . $lines[$i]->id . 'night_out_allowance" value="' . htmlspecialchars(GETPOST($lines[$i]->id . 'night_out_allowance')) . '"' . ($disabledtask ? ' disabled="disabled"' : '') . 'step="0.01" >';
				print '</input></div>';
				print '</td>';

				//Other
				// print '<td class="center">';
				// print '<div class="custom-input-group"><span class="input-group-text">R</span><input class="form-control" type="number name="' . $lines[$i]->id . 'other" rows="' . ROWS_2 . '" id="' . $lines[$i]->id . 'other" value="' . htmlspecialchars(GETPOST($lines[$i]->id . 'other')) . '"' . ($disabledtask ? ' disabled="disabled"' : '') . 'step="0.01">';
				// print '</input></div>';
				// print '</td>';

				//Other
				print '<td class="center">';
				print '<input name="' . $lines[$i]->id . 'other" rows="' . ROWS_2 . '" id="' . $lines[$i]->id . 'other" value="' . htmlspecialchars(GETPOST($lines[$i]->id . 'other')) . '"' . ($disabledtask ? ' disabled="disabled"' : '') . '>';
				print '</input>';
				print '</td>';

				//Other Comments
				print '<td class="center">';
				print '<input name="' . $lines[$i]->id . 'other_comments" rows="' . ROWS_2 . '" id="' . $lines[$i]->id . 'other_comments" value="' . htmlspecialchars(GETPOST($lines[$i]->id . 'other_comments')) . '"' . ($disabledtask ? ' disabled="disabled"' : '') . '>';
				print '</input>';
				print '</td>';
