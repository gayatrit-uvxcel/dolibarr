...
class pdf_cornas extends ModelePDFSuppliersOrders
{
            ...
            if (file_exists($dir)) {
                ...
                for ($i = 0; $i < $nblines; $i++) {
                    ...
                    if ($this->getColumnStatus('subprice')) {
                        ...
                        $up_excl_tax = str_replace(',', '', $up_excl_tax);
                        $this->printStdColumnContent($pdf, $curY, 'subprice', price($up_excl_tax,0,'outlangs',1,-1,-1,$conf->currency));
                        $nexY = max($pdf->GetY(), $nexY);
                    }
                ...
                $termsContentHeight = $this->displayTermsAndCondition($pdf, $object, true, $outputlangs);
                $remainingSpaceforTerms = $this->page_hauteur - $pdf->GetY() - $heightforfooter;
                if ($remainingSpaceforTerms > $termsContentHeight) {
                    $posy = $this->displayTermsAndCondition($pdf, $object, false, $outputlangs);
                } else {
                    $this->_pagefoot($pdf, $object, $outputlangs, 1);
                    // New page
                    $pdf->AddPage();
                ...
                if ($remainingforRegards > $heightOfRegards) {
                    $posy = $this->displayRegards($pdf, $object, false . $default_font_size, $default_font_size);
                } else {
                    $this->_pagefoot($pdf, $object, $outputlangs, 1);
                    // New page
                    $pdf->AddPage();
    ...
    protected function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
    {
        ...
        $pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities("Sub Total (excl. VAT)"), 0, 'L', 1);

        $total_ht = ((isModEnabled("multicurrency") && isset($object->multicurrency_tx) && $object->multicurrency_tx != 1) ? $object->multicurrency_total_ht : $object->total_ht);
        $pdf->SetXY($col2x, $tab2_top);
        $pdf->MultiCell($largcol2, $tab2_hl, price($total_ht + (!empty($object->remise) ? $object->remise : 0),0,'outlangs',0,-1,-1,$conf->currency), 0, 'R', 1);
        ...
        foreach ($this->tva as $tvakey => $tvaval) {
            if ($tvakey) { // On affiche pas taux 0
                ...
                $totalvat = $outputlangs->transcountrynoentities("VAT @", $mysoc->country_code) . ' ';
                $totalvat .= vatrate($tvakey, 1) . $tvacompl;
                $pdf->MultiCell($col2x - $col1x, $tab2_hl, $totalvat, 0, 'L', 1);

                $pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
                $pdf->MultiCell($largcol2, $tab2_hl, price($tvaval,0,'outlangs',0,-1,-1,$conf->currency), 0, 'R', 1);
            }
        }
        if (!$this->atleastoneratenotnull) { // If no vat at all
            ...
            if (!empty($conf->global->FACTURE_LOCAL_TAX2_OPTION) && $conf->global->FACTURE_LOCAL_TAX2_OPTION == 'localtax2on' && $object->total_localtax2 > 0) {
                $index++;
                $pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
                $pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transcountrynoentities("TotdalLT2", $mysoc->country_code), 0, 'L', 1);
                $pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
        ...
        $pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities("Total Price (incl. VAT)"), $useborder, 'L', 1);

        $total_ttc = (isModEnabled("multicurrency") && $object->multicurrency_tx != 1) ? $object->multicurrency_total_ttc : $object->total_ttc;
        $pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
        // $pdf->MultiCell($largcol2, $tab2_hl, price($total_ttc), $useborder, 'R', 1);

        $pdf->MultiCell($largcol2, $tab2_hl, price($total_ttc, 0, 'outlangs', 0, -1, -1, $conf->currency), $useborder, 'R', 1);
    ...
    protected function _pagehead(&$pdf, $object, $outputlangs, $pagenb)
    {
        ...
        $sql_llx_societe = "SELECT * FROM " . MAIN_DB_PREFIX . "societe WHERE rowid = $po_obj->company_rowid";
        $res_llx_societe = $this->db->query($sql_llx_societe);
        if ($res_llx_societe) {
            $row = $this->db->fetch_object($res_llx_societe);
            if ($row) {
                $po_obj->company = $row->nom;
                $po_obj->vendorVat = $row->client_vat;
        ...
        if ($pdf->getPage() === 1) {
            ...
            $pdf->Cell(100, 5, $po_obj->dateValue, 'T', 0, 'L', true);
            $pdf->Cell(30, 5, 'Division:', 'T', 0, 'R', true);
            $pdf->Cell(0, 5, $po_obj->division, 'TR', 1, 'L', true);


            // second row
            $pdf->Cell(30, 5, 'Company: ', 'L', 0, 'R', true);
            $pdf->Cell(100, 5, $po_obj->company, '', 0, 'L', true);
            $pdf->Cell(30, 5, 'Vendor No:', '', 0, 'R', true);
            $pdf->Cell(0, 5, $po_obj->vendorNO, 'R', 1, 'L', true);

            // third row
            $pdf->Cell(30, 5, 'Project: ', 'L', 0, 'R', true);
            $pdf->Cell(100, 5, $po_obj->project, '', 0, 'L', true);
            $pdf->Cell(30, 5, 'P.O. No.:', '', 0, 'R', true);
            $pdf->Cell(0, 5, $po_obj->poNo, 'R', 1, 'L', true);

            // fourth row
            $pdf->Cell(30, 5, 'Contact: ', 'L', 0, 'R', true);
            $pdf->Cell(100, 5, $po_obj->contact, '', 0, 'L', true);
            $pdf->Cell(30, 5, ' ', '', 0, 'R', true);
            $pdf->Cell(0, 5, '', 'R', 1, 'L', true);

            // fifth row
            $pdf->Cell(30, 5, 'Tel No: ', 'LR', 0, 'R', true);
            $pdf->Cell(100, 5, $po_obj->tellNo, '', 0, 'L', true);
            $pdf->Cell(30, 5, 'Quote No.: ', '', 0, 'R', true);
            $pdf->Cell(0, 5, $po_obj->quoteNo, 'R', 1, 'L', true);

            // sixth row
            $pdf->Cell(30, 5, 'Email: ', 'L', 0, 'R', true);
            $pdf->Cell(100, 5, $po_obj->email, '', 0, 'L', true);
            $pdf->Cell(30, 5, ' ', '', 0, 'R', true);
            $pdf->Cell(0, 5, '', 'R', 1, 'L', true);

            // seventh row
            $pdf->Cell(30, 5, 'Vendor Vat: ', 'LB', 0, 'R', true);
            $pdf->Cell(100, 5, $po_obj->vendorVat, 'B', 0, 'L', true);
            $pdf->Cell(30, 5, 'Our Vat No.: ', 'B', 0, 'R', true);
            $pdf->Cell(0, 5, $po_obj->ourVatNo, 'RB', 1, 'L', true);
        }
        return $posy;
    }
    ...
    public function displayNotes(&$pdf, $posy, $object, $outputlangs, $calculateHeightOnly = false)
    {
        ...
        if (!$calculateHeightOnly) {
            $pdf->SetY($pdf->GetY() + $marginTop);
             ...
             // Set font to bold for the title
             $pdf->SetFont('', 'b', $default_font_size);
             $pdf->MultiCell(0, $lineHeight, $title, 0, 'L', 0);
    ...
    public function displayTermsAndCondition(&$pdf, $object, $calculateHeightOnly = false, $outputlangs)
    {
        $lineHeight = 5;
        $marginTop = 8;
        $default_font_size = pdf_getPDFFontSize($outputlangs);
