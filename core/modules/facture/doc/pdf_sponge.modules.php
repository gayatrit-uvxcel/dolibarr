...
class pdf_sponge extends ModelePDFFactures
{
            ...
            if (file_exists($dir)) {
                ...
                for ($i = 0; $i < $nblines; $i++) {
                    ...
                    if ($this->getColumnStatus('desc')) {
                        ...
                        if (!isset($line->desc)) {  
                            $this->printStdColumnContent($pdf, $curY, 'desc', $line['desc'],$isCategory=true);
                        } else {
                            $this->printColDescContent($pdf, $curY, 'desc', $object, $i, $outputlangs, $hideref, $hidedesc,$isCategory=true);
                        }
                    ...
                    if (isset($line->subprice)) {
                        ...
                        $this->printStdColumnContent($pdf, $curY, 'subprice',  price($line->subprice, 0, '', 1, -1, -1, $conf->currency));
                    }
                    ...
                    if ($this->getColumnStatus('totalexcltax')) {
                        ...
                        $total_excl_tax = str_replace(',', '', $total_excl_tax);
                        // modification by sp start
                        if ($total_excl_tax != "0.00") {
                            // $this->printStdColumnContent($pdf, $curY, 'totalexcltax', $total_excl_tax);
                            $this->printStdColumnContent($pdf, $curY, 'totalexcltax', price($total_excl_tax, 0, 'outlangs', 1, -1, -1, $conf->currency));
                        }
                        // modification by sp end
                        $nexY = max($pdf->GetY(), $nexY);
                    }
                ...
                $heightOfRegards = $this->displayRegards($pdf, $object, true, $default_font_size);
                if ($remainingforRegards > $heightOfRegards) {
                    $posy = $this->displayRegards($pdf, $object, false, $default_font_size);
                } else {
                    $this->_pagefoot($pdf, $object, $outputlangs, 1);
                    // New page
                    $pdf->AddPage();
    ...
    protected function drawTotalTable(&$pdf, $object, $deja_regle, $posy, $outputlangs, $outputlangsbis)
    {
        ...
        $sql_llx_propal = "SELECT total_tva, localtax1, localtax2, total_ht, total_ttc FROM " . MAIN_DB_PREFIX . "propal WHERE fk_soc = $object->socid AND  fk_projet = $object->fk_project";
        ...
        if ($res_llx_propal) {
            while ($row = $this->db->fetch_object($res_llx_propal)) {
                ...
                // $object->localtax1 = $row->localtax1;
                // $object->localtax2 = $row->localtax2;
                $object->total_ttc = $row->total_ttc;
            }
        } else {
        ...
        $pdf->MultiCell($largcol2, $tab2_hl, price($sign * $object->total_ht, 0, $outputlangs, 1, -1, -1, $conf->currency), 0, 'R', 1);
        ...
        $pdf->MultiCell($largcol2, $tab2_hl, price($scheduleAmountHT, 0, 'outlangs', 1, -1, -1, $conf->currency), 0, 'R', 1);
        ...
        foreach ($this->tva_array as $tvakey => $tvaval) {
            if ($tvakey) { // On affiche pas taux 0
                ...
                $pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
                $multicurrency_total_tva = number_format(($scheduleAmountHT * $tvaval['vatrate']) / 100, 2, '.', '');

                // $pdf->MultiCell($largcol2, $tab2_hl, price($multicurrency_total_tva, 0, $outputlangs), 0, 'R', 1);
                $pdf->MultiCell($largcol2, $tab2_hl, price($multicurrency_total_tva, 0, 'outlangs', 0, -1, -1, $conf->currency), 0, 'R', 1);
            }
        }
        ...
        $pdf->MultiCell($largcol2, $tab2_hl, price($amountHTIncludingVat, 0, 'outlangs', 0, -1, -1, $conf->currency), $useborder, 'R', 1);
        $pdf->SetFont('', 'B', $default_font_size - 1);
        // $this->atleastoneratenotnull = 0;
        ...
        if (($deja_regle > 0 || $creditnoteamount > 0 || $depositsamount > 0) && empty($conf->global->INVOICE_NO_PAYMENT_DETAILS)) {
            ...
            $pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
            $pdf->MultiCell($largcol2, $tab2_hl, price($deja_regle + $depositsamount, 0, $outputlangs, 0, -1, -1, $conf->currency), 0, 'R', 0);
            ...
            $pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
            $pdf->MultiCell($largcol2, $tab2_hl, price($resteapayer, 0, $outputlangs, 0, -1, -1, $conf->currency), $useborder, 'R', 1);
    ...
    protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null, $pagenb)
    {
        ...
        $logo = 'C:/dolibarr/www/dolibarr/htdocs/install/doctemplates/websites/website_template-corporate/medias/image/websitekey/pdfLogo.png';
        // $logo = 'var/www/html/dolibarr/htdocs/install/doctemplates/websites/website_template-corporate/medias/image/websitekey/pdfLogo.png';
        if (is_readable($logo)) {
            $height = pdf_getHeightForLogo($logo);
            $logoWidth = 120;
            // Calculate the X coordinate to center the logo
        ...
        $sql_llx_societe = "SELECT * FROM " . MAIN_DB_PREFIX . "societe WHERE rowid = $invoice_obj->company_rowid";
        ...
        if ($pagenb === 1) {
            ...
            // first row
            $pdf->Cell(30, 5, 'Date: ', 'LT', 0, 'R', true);
            $pdf->Cell(100, 5, $invoice_obj->dateValue, 'T', 0, 'L', true);
            $pdf->Cell(30, 5, 'Division:', 'T', 0, 'R', true);
            $pdf->Cell(0, 5, $invoice_obj->division, 'TR', 1, 'L', true);

            // second row
            $pdf->Cell(30, 5, 'Company: ', 'L', 0, 'R', true);
            $pdf->Cell(100, 5, $invoice_obj->company, '', 0, 'L', true);
            $pdf->Cell(30, 5, 'Vendor No:', '', 0, 'R', true);
            $pdf->Cell(0, 5, $invoice_obj->vendorNO, 'R', 1, 'L', true);

            // third row
            $pdf->Cell(30, 5, 'Project: ', 'L', 0, 'R', true);
            $pdf->Cell(100, 5, $invoice_obj->project, '', 0, 'L', true);
            $pdf->Cell(30, 5, 'P.O. No.:', '', 0, 'R', true);
            $pdf->Cell(0, 5, $invoice_obj->poNo, 'R', 1, 'L', true);

            // fourth row
            $pdf->Cell(30, 5, 'Contact: ', 'L', 0, 'R', true);
            $pdf->Cell(100, 5, $invoice_obj->contact, '', 0, 'L', true);
            $pdf->Cell(30, 5, ' ', '', 0, 'R', true);
            $pdf->Cell(0, 5, '', 'R', 1, 'L', true);

            // fifth row
            $pdf->Cell(30, 5, 'Tel No: ', 'LR', 0, 'R', true);
            $pdf->Cell(100, 5, $invoice_obj->tellNo, '', 0, 'L', true);
            $pdf->Cell(30, 5, 'Quote No.: ', '', 0, 'R', true);
            $pdf->Cell(0, 5, $invoice_obj->quoteNo, 'R', 1, 'L', true);

            // sixth row
            $pdf->Cell(30, 5, 'Email: ', 'L', 0, 'R', true);
            $pdf->Cell(100, 5, $invoice_obj->email, '', 0, 'L', true);
            $pdf->Cell(30, 5, 'Invoice No.: ', '', 0, 'R', true);
            $pdf->Cell(0, 5, $invoice_obj->invoiceNo, 'R', 1, 'L', true);

            // seventh row
            $pdf->Cell(30, 5, 'Client Vat: ', 'LB', 0, 'R', true);
            $pdf->Cell(100, 5, $invoice_obj->clientVat, 'B', 0, 'L', true);
            $pdf->Cell(30, 5, 'Our Vat No.: ', 'B', 0, 'R', true);
            $pdf->Cell(0, 5, $invoice_obj->ourVatNo, 'RB', 1, 'L', true);
        }
    ...
    public function bankDetails(&$pdf, $posy, $outputlangs, $calculateHeightOnly = false)
    {
        ...
        if (!$calculateHeightOnly) {
            ...
            $pdf->Cell(40, $lineHeight, 'Standard Bank', 1, 0, 'L');
            $pdf->Cell(40, $lineHeight, 'Branch Code', 1, 0, 'L');
            $pdf->Cell(0, $lineHeight, '016 342, Greenstone', 1, 1, 'L');

            // second row
            $pdf->Cell(40, $lineHeight, 'Account Name', 1, 0, 'L');
            $pdf->Cell(40, $lineHeight, 'First Vision Automation', 1, 0, 'L');
            $pdf->Cell(40, $lineHeight, 'Account type', 1, 0, 'L');
            $pdf->Cell(0, $lineHeight, 'Current', 1, 1, 'L');

            // third row
            $pdf->Cell(40, $lineHeight, 'Account No.', 1, 0, 'L');
            $pdf->Cell(40, $lineHeight, '22 01 34 545', 1, 0, 'L');
            $pdf->Cell(40, $lineHeight, 'VAT No.', 1, 0, 'L');
            $pdf->Cell(0, $lineHeight, '4580291799', 1, 1, 'L');
        }
        $finalY = $pdf->GetY();
    ...
    // to display regards
    public function displayRegards(&$pdf, $object, $calculateHeightOnly = false, $default_font_size)
    {
        // $contentHeight;
        $initialY = $pdf->GetY();
        $marginTop = 3;
