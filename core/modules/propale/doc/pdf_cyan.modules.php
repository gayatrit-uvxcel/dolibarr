...
class pdf_cyan extends ModelePDFPropales
{
            ...
            if (file_exists($dir)) {
                ...
                for ($i = 0; $i < $nblines; $i++) {
                    ...
                    if (isset($line->subprice)) {
                        // $this->printStdColumnContent($pdf, $curY, 'subprice', 'R ' . number_format($line->subprice, 2));
                    ...
                    if ($this->getColumnStatus('totalexcltax')) {
                        ...
                        if ($total_excl_tax != "0.00") {
                            $this->printStdColumnContent($pdf, $curY, 'totalexcltax', price($total_excl_tax, 0, 'outlangs', 0, -1, -1, $conf->currency));
                        }
                        $nexY = max($pdf->GetY(), $nexY);
                    }
    ...
    protected function drawTotalTable(&$pdf, $object, $deja_regle, $posy, $outputlangs, $outputlangsbis = null)
    {
        ...
        $pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities("Sub Total (excl. VAT)") . (is_object($outputlangsbis) ? ' / ' . $outputlangsbis->transnoentities("Sub Total (excl. VAT)") : ''), 0, 'L', 1);

        $total_ht = ((isModEnabled("multicurrency") && isset($object->multicurrency_tx) && $object->multicurrency_tx != 1) ? $object->multicurrency_total_ht : $object->total_ht);
        $pdf->SetXY($col2x, $tab2_top);

        $pdf->MultiCell($largcol2, $tab2_hl, price($total_ht + (!empty($object->remise) ? $object->remise : 0),0,'outlang',1,-1,-1,$conf->currency), 0, 'R', 1);
            ...
            if (!empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_IFNULL) && $tvaisnull) {
                ...
                foreach ($this->tva as $tvakey => $tvaval) {
                    if ($tvakey) { // On affiche pas taux 0
                        ...
                        $pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
                        // $pdf->MultiCell($largcol2, $tab2_hl, 'R ' . price($tvaval, 0, $outputlangs), 0, 'R', 1);
                ...
                $pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
                // $pdf->MultiCell($largcol2, $tab2_hl, 'R ' . price($total_ttc, 0, $outputlangs), $useborder, 'R', 1);
    ...
    protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null, $pagenb)
    {
        ...
        if ($res_llx_facture) {
            while ($row = $this->db->fetch_object($res_llx_facture)) {
                $originalDate = $row->dateOfProposal;
