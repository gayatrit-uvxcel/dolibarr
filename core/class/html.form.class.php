...
class Form
{
    ...
    public function select_type_of_category($selected = '', $htmlname = 'category', $showempty = 0, $hidetext = 0, $forceall = 0)
    {
        ...
        if ($res_llx_propaldet_categories) {
            while ($row = $this->db->fetch_object($res_llx_propaldet_categories)) {
                ...
                if ($row->main_category_name !== '' && $row->category_name !== '') {
                    $categoryArray[] = $row->main_category_name . " - " . $row->category_name;
                } elseif ($row->main_category_name !== '' && $row->category_name === '') {
                    $categoryArray[] = $row->main_category_name;
                } elseif ($row->category_name !== '' && $row->main_category_name === '') {
                    $categoryArray[] = $row->category_name;
                }
            }
        }

        $maincategoryvalue = GETPOST('main_category_name') ? preg_replace('/\s+/', ' ', GETPOST('main_category_name')) : null;
        $subcategoryvalue = GETPOST('subcategory_name') ?  preg_replace('/\s+/', ' ', GETPOST('subcategory_name')) : null;
    ...
    // public function form_terms_conditions($page, $selected = '', $htmlname = 'terms_and_conditions')
    // {
    ...
    //     // Fetch all available terms and conditions
    ...
    //         }
    //     } else {
    //         echo "Error executing terms_conditions query: " . $db->lasterror();
    //     }

    //       //Convert the selected terms (if provided) into an array if they are not already
    //     if (!is_array($selected)) {
    //         $selected = json_decode($selected, true); // Assuming the $selected value comes as a JSON array of terms
    //     }
    //     if (!$selected) {
    //         $selected = array(); // Ensure $selected is always an array
    //     }

    //     if ($htmlname != "none") {
    //         print '<form method="POST" action="' . $page . '">';
    //         print '<input type="hidden" name="action" value="settermsandconditions">';
    //         print '<input type="hidden" name="token" value="' . newToken() . '">';
    //         print $this->multiselectarray('multi_tc_content', $options, $selected, 0, 0, 'form-control', 1, '300px', '', '', 'Select Multiple Terms and Conditions', 1);
    //         print '<input type="submit" class="button smallpaddingimp valignmiddle" value="' . $langs->trans("Modify") . '">';
    //         print '</form>';
    //     } else {
    //         dol_include_once('/core/lib/company.lib.php');
    ...
    public function form_terms_conditions($page, $selected = '', $htmlname = 'terms_and_conditions')
    {
        ...
        if ($htmlname != "none") {
            print '<form method="POST" action="' . $page . '">';
            print '<input type="hidden" name="action" value="settermsandconditions">';
            print '<input type="hidden" name="token" value="' . htmlspecialchars(newToken()) . '">';
            print $this->multiselectarray('multi_tc_content', $options, $selected, 0, 0, 'form-control', 1, '300px', '', '', 'Select Multiple Terms and Conditions', 1);
            ...
            dol_include_once('/core/lib/company.lib.php');
            print !empty($selected) ? htmlspecialchars(currency_name($selected, 1)) : '&nbsp;';
        }
    }

    public function form_terms_conditions_po($page, $selected = '', $htmlname = 'terms_and_conditions')
    {
        // Declare global variables
        global $db, $langs;
    ...
    public function form_notes($page, $selected = '', $htmlname = 'notes')
    {
        // Declare global variables
        global $db, $langs;
        ...
        if ($htmlname != "none") {
            ...
            print '<input type="hidden" name="token" value="' . htmlspecialchars(newToken()) . '">';
            print $this->multiselectarray('notes_content', $options, $selected, 0, 0, 'form-control', 1, '300px', '', '', 'Select Multiple Notes', 1);
    ...
    public function form_notes_po($page, $selected = '', $htmlname = 'notes')
    {
        // Declare global variables
        global $db, $langs;
    ...
    public function select_duration($prefix, $iSecond = '', $disabled = 0, $typehour = 'select', $minunderhours = 0, $nooutput = 0)
    {
        ...
        // $hourSelected = '';
        // $minSelected = '';
        $hourSelected = GETPOST($prefix . 'hour');
        $minSelected = GETPOST($prefix . 'min');
    ...
    public static function multiselectarray($htmlname, $array, $selected = array(), $key_in_label = 0, $value_as_key = 0, $morecss = '', $translate = 0, $width = 0, $moreattrib = '', $elemtype = '', $placeholder = '', $addjscombo = -1)
    {
        ...
        // Then add a script to handle selections
        // $out .= '<script>
        //         $(document).ready(function() {
        //             let selectionOrder = [];
        ...
        //             $("#' . $htmlname . '").on("select2:select select2:unselect", function(e) {
        //                 const selectedValue = e.params.data.text; // Get the selected or unselected value

        //                 // Check if the value is being added or removed
        //                 const index = selectionOrder.indexOf(selectedValue);

        //                 if (index === -1) {
        //                     // Value is being added
        //                     selectionOrder.push(selectedValue);
        //                 } else {
        //                     // Value is being removed
        //                     selectionOrder.splice(index, 1);
        //                 }

        //                 // Log the current selection order
