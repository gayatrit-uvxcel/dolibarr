...
abstract class CommonObject
{
    ...
    public function setTermsAndConditions($code)
    {
        dol_syslog(get_class($this) . '::setTermsAndConditions(' . $code . ')');
        if ($this->statut >= 0 || $this->element == 'societe') {
            ...
            if ($this->db->query($sql)) {
                $this->terms_and_conditions = $code;
                ...
                dol_syslog(get_class($this) . '::setTermsAndConditions Error ' . $sql . ' - ' . $this->db->error());
                $this->error = $this->db->error();
                return -1;
            }
        } else {
            dol_syslog(get_class($this) . '::setTermsAndConditions, status of the object is incompatible');
            $this->error = 'Status of the object is incompatible ' . $this->statut;
            return -2;
        }
    }

    public function setNote($code)
    {
        dol_syslog(get_class($this) . '::setNote(' . $code . ')');
        if ($this->statut >= 0 || $this->element == 'societe') {
            $fieldname = 'notes';
            ...
            if ($this->db->query($sql)) {
                ...
                dol_syslog(get_class($this) . '::setNote Error ' . $sql . ' - ' . $this->db->error());
                $this->error = $this->db->error();
                return -1;
            }
        } else {
            dol_syslog(get_class($this) . '::setNote, status of the object is incompatible');
            $this->error = 'Status of the object is incompatible ' . $this->statut;
            return -2;
        }
    }
    ...
    public function addThumbs($file)
    {
        ...
        if (file_exists($file_osencoded)) {
            ...
            // Create small thumbs for company
            vignette($file_osencoded, $maxwidthsmall, $maxheightsmall, '_small', $quality);
            $results[] = $file_osencoded . '_small'; // Store path of the small thumbnail

            // Create mini thumbs for company
            vignette($file_osencoded, $maxwidthmini, $maxheightmini, '_mini', $quality);
            $results[] = $file_osencoded . '_mini'; // Store path of the mini thumbnail
        }

        return $results; // Return the paths of created thumbnails
    }
