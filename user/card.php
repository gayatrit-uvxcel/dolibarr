...
// code added by gt start

$datestartvalidity = dol_mktime(0, 0, 0, 9, 20, 2024);
$dateendvalidity = dol_mktime(0, 0, 0, 10, 31, 2024);
...
if (empty($reshook)) {
	...
	if ($action == 'add' && $canadduser) {
		...
		if (!$error) {
			...
			// code added by gt for profile pic
			...
			// code added by gt for profile pic end
			// Set entity property
			...
			if (!$error && !count($object->errors)) {
				if (GETPOST('deletephoto') && $object->oldcopy->photo) {
					$fileimg = $conf->user->dir_output . '/' . $id . 'photos/' . $object->oldcopy->photo;
					$dirthumbs = $conf->user->dir_output . '/' . $id . 'photos/thumbs';
				...
				if (isset($_FILES['photo']['tmp_name']) && trim($_FILES['photo']['tmp_name'])) {
					$dir = $conf->user->dir_output . '/' . $id . '/photos';
					dol_mkdir($dir);
...
if ($action == 'create' || $action == 'adduserldap') {
	...
	print '<form action="' . $_SERVER['PHP_SELF'] . '" method="POST" name="createuser" enctype="multipart/form-data">';
	print '<input type="hidden" name="token" value="' . newToken() . '">';
			...
			*/
			...
			if (empty($reshook)) {
				...
				if ($caneditfield && (!isModEnabled('multicompany') || !$user->entity || ($object->entity == $conf->entity) || ($conf->global->MULTICOMPANY_TRANSVERSE_MODE && $object->entity == 1))) {
					...
					$caneditpassword && !$object->ldap_sid &&
