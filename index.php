...
if ($projectid > 0) {
    ...
    if ($object->quote_no && $object->invoice_no) {
        ...
        $categoryArray = [];
        $sql_llx_propaldet_categories = "SELECT DISTINCT SUBSTRING_INDEX(category, ' - ', 1) AS main_category FROM " . MAIN_DB_PREFIX . "facturedet WHERE category IS NOT NULL AND fk_projectid=$projectid";
        ...
        if (isset($categoryArray) && is_array($categoryArray) && !empty($categoryArray)) {
            ...
            foreach ($categoryArray as $category) {
                ...
                $sql_llx_propaldet_total = "SELECT SUM(total_ht) As sumOfTotalPrice FROM " . MAIN_DB_PREFIX . "facturedet WHERE SUBSTRING_INDEX(category, ' - ', 1) = '$category' AND fk_socid = $socid AND fk_projectid = $projectid";
